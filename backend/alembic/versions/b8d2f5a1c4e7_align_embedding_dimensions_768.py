...
def _retype_vector_column(table: str, column: str, from_dim: int, to_dim: int) -> None:
    op.execute(
        f"""
        DO $$
        BEGIN
            IF EXISTS (
                ...
                -- Drop mismatched vectors: they cannot be cast across dimensions.
                UPDATE {table}
                   SET {column} = NULL
                 WHERE {column} IS NOT NULL
                   AND vector_dims({column}) <> {to_dim};
...
def upgrade() -> None:
    ...
    _retype_vector_column("normalized_alerts", "embedding", 384, TARGET_DIM)
    op.execute(
        f"""
        DO $$
        BEGIN
    ...
    _retype_vector_column("alert_embeddings", "embedding", 1536, TARGET_DIM)


def downgrade() -> None:
    op.execute(f"DROP INDEX IF EXISTS {HNSW_INDEX};")
    _retype_vector_column("normalized_alerts", "embedding", TARGET_DIM, 384)
    _retype_vector_column("alert_embeddings", "embedding", TARGET_DIM, 1536)
    op.execute(
        f"""
        DO $$
        BEGIN
